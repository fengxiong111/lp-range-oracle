...
function replay(raw:RawCandidate,candles:Ohlcv[],price:number,feeRate:number|null):Replay{
  ...
  let weightedVolume=0,weightedIn=0,timeWeight=0,activeWeight=0,rawIn=0,transitions=0,transitionWeight=0;
  ...
  for(const c of ordered){
    ...
    if(previousInside!==null&&inside!==previousInside){transitions+=1;transitionWeight+=decay;}
  ...
  return {
    sampleHours,
    observedVolumeUsd:rawIn,
    feeProxyUsd:feeRate===null?null:rawIn*feeRate,
    weightedVolumeCapturePct:capture*100,
    activeTimePct:active*100,
    crossingDensity:crossing*100,
    density,capture,active,crossing,
  };
}
...
function scoreRows(rawRows:RawCandidate[],candles:Ohlcv[],price:number,feeRate:number|null):Array<{raw:RawCandidate;replay:Replay;score:number}>{
  ...
  return evaluated.map(x=>{
    const densityNorm=clamp(x.replay.density/maxDensity,0,1);
    const r=x.replay;
    const score=x.raw.kind==="CORE"
      ?100*(.45*densityNorm+.25*r.capture+.20*r.active+.10*r.crossing)
      :100*(.20*densityNorm+.35*r.capture+.35*r.active+.10*r.crossing);
...
export function searchRanges(t:TruthHandoff):{candidates:RangeCandidate[];core:RangeCandidate|null;buffer:RangeCandidate|null}{
  ...
  const positiveVolumes=ordered.map(c=>Math.max(0,c.volumeUsd||0)).filter(v=>v>0).sort((a,b)=>a-b);
  const medianVol=positiveVolumes.length?positiveVolumes[Math.floor(positiveVolumes.length/2)]:1;
  ...
  const q=(p:number)=>weightedQuantile(samples,p);
  ...
  const trendBase=last24.length>1?last24[0].close:null; const trend=trendBase&&trendBase>0?last24.at(-1)!.close/trendBase-1:0;
  const down=trend<-.03,up=trend>.03;
  const skewLower=price*(1-volWidth*(down?1.35:up?.75:1)); const skewUpper=price*(1+volWidth*(up?1.35:down?.75:1));

  const coreRaw=dedupe([
    normalizeRange("CORE","VOLUME_Q25_Q75",q(.25),q(.75),price),
    normalizeRange("CORE","VOLUME_Q20_Q80",q(.20),q(.80),price),
    normalizeRange("CORE","VOLUME_Q15_Q85",q(.15),q(.85),price),
    normalizeRange("CORE","RECENT_24H_ENVELOPE",low24,high24,price),
    normalizeRange("CORE",`VOLATILITY_SKEW_${down?"DOWN":up?"UP":"NEUTRAL"}`,skewLower,skewUpper,price),
  ].filter((x):x is RawCandidate=>Boolean(x)));
  const feeRate=t.market.feeTier===null?null:t.market.feeTier/1_000_000;
  const coreScored=scoreRows(coreRaw,ordered,price,feeRate); const winningCore=coreScored[0]??null;

  const expand=winningCore?Math.max((winningCore.raw.upper-winningCore.raw.lower)*.25,price*.04):price*.10;
  const bufferRaw=dedupe([
    normalizeRange("BUFFER","VOLUME_Q05_Q95",q(.05),q(.95),price),
    normalizeRange("BUFFER","VOLUME_Q02_Q98",q(.02),q(.98),price),
    normalizeRange("BUFFER","RECENT_24H_SAFETY",low24===null?null:low24*.94,high24===null?null:high24*1.06,price),
    winningCore?normalizeRange("BUFFER","CORE_PLUS_REVISIT_MARGIN",winningCore.raw.lower-expand,winningCore.raw.upper+expand,price):null,
  ].filter((x):x is RawCandidate=>Boolean(x))).filter(b=>!winningCore||(b.lower<=winningCore.raw.lower&&b.upper>=winningCore.raw.upper));
  const bufferScored=scoreRows(bufferRaw,ordered,price,feeRate); const winningBuffer=bufferScored[0]??null;

  const convert=(x:{raw:RawCandidate;replay:Replay;score:number},selected:boolean):RangeCandidate=>({kind:x.raw.kind,strategy:x.raw.strategy,lowerPriceUsd:x.raw.lower,upperPriceUsd:x.raw.upper,widthPct:(x.raw.upper-x.raw.lower)/price*100,score:x.score,selected,replay:{sampleHours:x.replay.sampleHours,observedVolumeUsd:x.replay.observedVolumeUsd,feeProxyUsd:x.replay.feeProxyUsd,weightedVolumeCapturePct:x.replay.weightedVolumeCapturePct,activeTimePct:x.replay.activeTimePct,crossingDensity:x.replay.crossingDensity},failureState:null});
  const cores=coreScored.map((x,i)=>convert(x,i===0)); const buffers=bufferScored.map((x,i)=>convert(x,i===0));
  return {candidates:[...cores,...buffers],core:cores[0]??null,buffer:buffers[0]??null};
}
